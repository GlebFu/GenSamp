...
load("Data/Population Data/Sim Data.rdata")
```
...
cluster.vars <- c(covariates[!(covariates %in% c("Urban", "Suburban", "ToRu"))], "urbanicity")
...
# undebug(get_CH)
save(clusters, distance, file =  "Data/Cluster Analysis/cluster analysis data - OV.rdata")
save(chPlot, clust_time, ch_time, K, cluster.vars, minK, df.clusters, cluster.vratio, file =  "Data/Cluster Analysis/Clusters - OV.rdata")
```

```{r SUBS_FULL_CH}
# load("clusters-full.rdata")
load("Data/Cluster Analysis/cluster analysis data - OV.rdata")
...
data.frame(k = minK:K, var = cluster.vratio, min80 = sum(cluster.vratio <= .8) + 1.5) %>%
