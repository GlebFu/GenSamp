...
For a given recruitment method, let $\psi_j$ represent the sample selection propensity score, or the logistic transform of the probability that school $j$ was included in an obtained sample. We estimate $\psi_j$ using a basic logistic regression:
\begin{align}
\label{eq:PSA}
\psi_j = \gamma_0 + \mathbf{x}_j \boldsymbol\gamma
\end{align}
Note that this sample selection propensity score is distinct from the potential participation propensity model (Equation \@ref(eq:RGM)), in that $\psi_j$ is based on a sample actually obtained under a given recruitment method, allowing for units to self-select or decline to participate. The estimated values of the sample selection propensity score, $\hat\psi_j$, serve as scalar summaries of differences across all covariates in $\mathbf{x}_j$.
