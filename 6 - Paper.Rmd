...
# figs$smd.label <- df.list$smd.results %>%
#   filter(SB == 1,
#          vnames %in% c("n")) %>%
#   group_by(across(any_of(vars$methods))) %>%
#   summarise(Value = mean(Value), .groups = "drop") %>%
#   arrange(Value) %>%
#   mutate(Value = seq(.1, .5, .1),
#          SB = 1,
#          alpha.group = 1) %>%
#   full_join(df.list$smd.results %>% select(RR) %>% unique, by = character()) %>%
#   plot_smd() %>% legend_plot
...
```{r}
smd.example.list <-
  list(tfun(test$plots[[1]]), 
       tfun(test$plots[[2]]), 
       tfun(test$plots[[3]]))


test.legend <- (smd.example.list[[1]] + 
