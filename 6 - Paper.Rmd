---
title             : "Assessing sampling methods for generalization from multi-site randomized trials: Modeling recruitment and participation"
...
load("Data/Paper/Paper Data 02-08-2021.rdata")
...
A shortcoming of retrospective generalization methods is under-coverage [@grovesSurveyMethodology2004; @chan_partially_2017] which occurs when a sample lacks units corresponding to a sub-group of the inference population. Under-coverage can be assessed using several techniques that identify how well a sample would generalize to a specific population [@stuartUsePropensityScores2011; @tiptonHowGeneralizableYour2014]. If under-coverage is great enough to prevent use of statistical adjustment for estimating the PATE, then the inference population must be redefined. However, re-defining the inference population diminishes the relevance of study results, undermining the benefits of large-scale multi-site trials.

## Stratified balanced sampling {-}

Rather than relying solely on retrospective adjustments, researchers can also address generalizability during the design and recruitment stages of a study, using robust sampling methods to obtain more representative samples [@tiptonImprovingGeneralizationsExperiments2013; @tiptonStratifiedSamplingUsing2014]. Like retrospective adjustment, these methods also require extant data on a well-defined and enumerated population.
One method in particular, Stratified Balanced Sampling (SBS), has received attention from intervention effectiveness researchers due to its accessibility and relevance to trials of educational interventions [@tipton_review_2018].
...
SBS is also limited in several respects. The method depends on the existence of a rich set of observed covariates related to treatment heterogeneity and sample selection for each site in the population. Readily available extant data consist primarily of demographics and may not contain information on covariates that are more proximally related to variation in treatment effects, which can result in omitted variable bias [@tiptonStratifiedSamplingUsing2014]. Additionally, implementing SBS may require more resources than a simple convenience sample because recruiting ranked sites from multiple strata requires a coordinated effort between recruiters [@tiptonImplicationsSmallSamples2017; @tiptonImprovedGeneralizabilityImproved2019]. This means that recruiters cannot work independently and must work in coordination with methodologists to implement the sampling strategy. 
...
Following @tiptonStratifiedSamplingUsing2014, we assume that the researcher's goal is to select a sample of $n$ sites such that there is balance along $\mathbf{x}_j$ between the sample and the population, indicating that the population is compositionally represented by the sample.
...
\end{align}
where $\bar{x}_p$ is the sample mean of covariate $p$, $\mu_p$ is the covariate mean in the population, and $\sigma_p$ is the standard deviation of the covariate in the population. $SMD$ values closer to zero indicate greater balance between the sample and the population. By this measure, obtaining a $SMD_p \le 0.25$ for all covariates in $P$ would indicate that the sample is well balanced and can be generalized to the population. Covariates for which $SMD_p > 0.25$ would likely require additional statistical adjustment in the final analysis.\JEP{Citations for these conventions?}
...
\end{equation}
Under this assumption, $E_j$ can be interpreted as an indicator for _potential response_, or whether a site would participate in the study _if approached for recruitment_, for any site in the population. 
This simplifying assumption could fail to hold if the likelihood that a site responds depends on the manner of recruitment.[^recruitment]

[^recruitment]: For example, a school site be more likely to respond to a recruitment attempt made through a face-to-face meeting than one from an email blast.
...
\end{equation}
This model is again a stylized representation of a real recruitment process, but still one that we think is useful to study.[^total-contacts]  

[^total-contacts]: In practice, recruitment occurs over time and recruiters may be in contact with multiple potential sites at any one time [@tiptonImprovedGeneralizabilityImproved2019]. This would suggest that the total number of contacts in a real recruitment process might exceed $R(S)$.
...
To model stratified sampling methods, we will assume that the population is divided into a set of $K$ strata. Proportional allocation will be used to ensure the relative size of the strata in the sample reflects that of the population [@tiptonStratifiedSamplingUsing2014]. 
...
This property allows us to manipulate the parameters of the response propensity model (Equation \@ref(eq:RGM)) without altering the behavior of the convenience sampling process.
...
Following @tiptonStratifiedSamplingUsing2014, we operationalize balanced sampling by generating ranks that prioritize the recruitment of sites based on their similarity to the "average" site in each stratum. 
...
\end{align}
where $w_p$ is a weight assigned to covariate $X_p$. In the absence of information about predictors of treatment effect heterogeneity, we set the weight of each covariate equal to the inverse of its population variance as $w_p = 1/\sigma^2_p$.[^weights]

[^weights]: In some contexts, researchers may prefer to use weights that put more emphasis on covariates thought to be more related to treatment effect heterogeneity. @tiptonStratifiedSamplingUsing2014 recommends this method if the relationship between variables and participation is unknown. @tipton_review_2018 recommend setting weights to 1 so that covariates with more variance drive stratification.
...
Scores within strata are based on equation \@ref(eq:euclid), where sites closer to the "center" of the stratum are more representative and are therefore a higher priority. Though unlikely, it is possible that several sites could be equidistant from the center of the stratum; in such cases, sites are ordered randomly. Because @tiptonStratifiedSamplingUsing2014 proposed balanced sampling in connection with stratification based on a cluster analysis, we only consider the stratified version, SBS.
...
For illustrative purposes, we selected as covariates school characteristics that previous research has found to be associated with participation in randomized trials [@stuartCharacteristicsSchoolDistricts2017; @fellersDevelopingApproachDetermine2017; @tiptonSiteSelectionExperiments2016]. These studies found that districts and schools with higher proportions of students who are English language learners, economically disadvantaged, non-White, and living in urban settings are more likely to participate.

We developed a sampling frame for this population using data from the Common Core of Data (CCD; https://nces.ed.gov/ccd/index.asp). The CCD is a comprehensive database housing annually collected census data of all public schools and districts. Table \@ref(tab:tab-RGM-Pars) displays population descriptives based on the selected covariates. Prior to stratification, we log-transformed school size (number of students), district size (number of schools) and the student-to-teacher ratio in order to allow proportional comparisons at the extremes of the distributions [@hennigHowFindAppropriate2013]. Figure \@ref(fig:fig-dist1) displays the distribution of the continuous variables used in the cluster analysis. In all, the population frame consisted of 6 states, 2,016 districts and 9,792 schools.

```{r tab-RGM-Pars}
tabs$pop.desc %>%
  select(Variable = Variables, M, SD, `Log-odds of participation` = SF100) %>%
  kable(
    booktabs = T,
    caption = "Descriptive statistics for the population of 9,792 schools and assumed log odds of participation associated with each covariate",
    col.names = c("Variables", "M", "SD", "Log-odds of participation")
  ) %>%
  kable_styling(
...
(ref:fig-dist1-caption) Distributions of continuous covariates. District Size, School Size, and Student/Teacher Ratio were transformed by taking natural logs.

```{r fig-dist1, fig.pos = "tbh", fig.width = 8, fig.height = 6, fig.cap = "(ref:fig-dist1-caption)"}
...
Our cluster analysis generally followed the approach and recommendations described by @tiptonStratifiedSamplingUsing2014, with a few modifications. Specifically, we implemented k-medoids clustering to divide the population into heterogeneous strata comprised of homogeneous sites. We used Gower's general similarity measure, which is appropriate for covariates that include both continuous and categorical variables [@gowerGeneralCoefficientSimilarity1971; @tiptonStratifiedSamplingUsing2014].
All analyses were performed in R [@rcite] using the _cluster_ package [@machlerClusterClusterAnalysis2012].
...
The strata ranged in size from `r format(min(tabs$strata.data$n), big.mark = ",")` to  `r format(max(tabs$strata.data$n), big.mark = ",")` schools and explained `r tabs$strata.data$bet.perc %>% unique() %>% round()`% of the variation across all covariates. The proportion of variance explained by the strata differed by covariate, with five covariates having between 0 and 11% variance explained by the strata and eight covariates having from 45% to 87% variance explained. This discrepancy suggests that stratification will not be equally useful for balancing all covariates.\JEP{Could you add the percentage of variance explained for each covariate as an additional column in Table 1?}

## Participation Propensity Score {-}

In order to simulate the full process of recruiting a sample for a multi-site trial, we need to choose parameter values for the participation propensity score model (Equation \@ref(eq:RGM)).
To do so, we drew on work by @fellersDevelopingApproachDetermine2017, who compared 571 elementary schools that participated in IES-funded studies to the full population of U.S. elementary schools on the same variables used in our sampling frame. 
Note that this comparison is between schools that participated in trials (i.e., $P_j = 1$) and the full population of schools, and so differences may be the result of _both_ non-response bias and researcher recruitment bias. 
In contrast, we are ideally interested in $\Pr(E_j = 1 | \mathbf{x}_j)$, the _potential_ participation probabilities across the full population.
However, there is little data currently available to directly inform this participation model, and so we relied on the findings of @fellersDevelopingApproachDetermine2017 to make tentative assumptions about the magnitude of the participation propensity model coefficients.

@fellersDevelopingApproachDetermine2017 reported absolute standardized mean differences between schools that participated in IES-funded randomized trials and the population. We standardized the covariates and used reported SMDs as coefficients in Equation \@ref(eq:RGM) to generated $\pi_j$ values. The coefficients are reported in Table \@ref(tab:tab-RGM-Pars).
...
There are several methods to quantify how generalizable a sample is to a target population. One common method is to compare the sample to the population on a range of covariates by examining SMDs as defined in Equation \@ref(eq:SMD). This method is limited as it only provides us with a measure of how close the sample means are to the population means.
...
$$
Note that this sample selection propensity score is distinct from the potential participation propensity model (Equation \@ref(eq:RGM)), in that $\psi_j$ is based on a sample actually obtained under a given recruitment method, allowing for units to self-select or decline to participate. For each recruitment method and each obtained sample, we estimated $\psi_j$ using a basic logistic regression:
\begin{align}
\label{eq:PSA}
...
Figure \@ref(fig:fig-avg-B) displays the average $B$-index resulting from each recruitment method, for varying participation rates (on the horizontal axis) and selection factors.\JEP{Use bubble labels in the plots and remove the figure legend.} Acceptable values of $B$ for generalizability vary depending on the size of the sample, the size of the population, and the number of covariates [@tiptonHowGeneralizableYour2014]. Given our design, a value of $B = .95$ would indicate very good overlap with no need to adjust estimates. Generally $B >= .80$ still supports generalizability with adjustment and likely no need to redefine the population. 

(ref:fig-avg-B-caption) Average $B$-index for varying participation rates across SFs, by sampling method. Horizontal dotted lines represented indices of .80 and .95, indicating acceptable or high levels of generalizability.

```{r fig-avg-B, fig.height = 4, fig.width = 8, fig.cap = "(ref:fig-avg-B-caption)"}
...
(ref:fig-SMD-groups-caption) Patterns of relative performance based on standardized mean differences. The dotted horizontal line represents a threshold of 0.25 for acceptable balance. Results are displayed for the highest selection factor $(SF = 1)$ to highlight differences between the groups of covariates.
...
(1) covariates where all methods consistently achieved good balance;
(2) covariates where stratified sampling methods consistently achieved good balance, but where unstratified methods sometimes resulted in poor balance; 
(3) covariates where good balance was consistently achieved for SBS only, and where other stratified methods outperformed the unstratified counterparts; and  
(4) covariates exhibiting unique patterns that did not match groups 1, 2 and 3. 

Figure \@ref(fig:fig-SMD-groups) displays an example of each pattern for $SF = 1$;\JEP{Use bubble labels in the plots and remove the figure legend. Also, could you add the specific covariate in the label for each group? e.g., "Group 1: Student/Teacher Ratio"} results for each specific covariate can be found in Online Appendix Figures \@ref(fig:fig-smd-group1), \@ref(fig:fig-smd-group2), \@ref(fig:fig-smd-group3), and \@ref(fig:fig-smd-group4).

(ref:fig-Var-Coef-caption) Variance explained by clustering vs absolute log-odds. Shaded areas illustrate groupings of covariates with respect to patterns of imbalance as measured by standardized mean differences.

```{r fig-Var-Coef, fig.width = 9, fig.height = 4, fig.cap = "(ref:fig-Var-Coef-caption)"}
...
plot_grid(plotlist = responses.fig.list,
          ...
          label_size = 10,
          align = "v") +
  theme(plot.margin = margin(0, 0, 24, 6)) +
...
```{r fig-gini, fig.width = 9, fig.height = 4, fig.cap = "(ref:fig-gini-caption) "}
...
## Stratification with Cluster Analysis {#stratification-cluster}
...
Selecting an appropriate number of clusters is one of the more difficult problems in cluster analysis [@steinleyKmeansClusteringHalfcentury2006]. @hennigHowFindAppropriate2013 argued that the method of selecting $K$ should depend on the context of the clustering, framing the issue as one of obtaining an appropriate subject-matter-dependent definition rather than a purely statistical question. Similarly, @tiptonStratifiedSamplingUsing2014 recommended using both empirical and practical criteria to select $K$. 
...
```{r fig-k-plots, fig.height = 7, fig.width = 8, out.width = "1\\linewidth", fig.pos = "p", fig.cap = "(ref:fig-k-plots-caption) "}
...
Considering both the statistical criteria and the pragmatic constraints, we generated 5 clusters for purposes of stratification. The strata resulting from this cluster analysis range in size from `r format(min(tabs$strata.data$n), big.mark = ",")` to  `r format(max(tabs$strata.data$n), big.mark = ",")` schools. The strata explain `r tabs$strata.data$bet.perc %>% unique()`% of the variation across all covariates (Figure \@ref(fig:fig-k-plots)b), with a CH-index of `r tabs$strata.data$ch %>% unique()` (Figure \@ref(fig:fig-k-plots)a). We also computed variance explained for individual covariates. The proportion of variance explained by the strata differed greatly by covariate, with 5 covariates having between 0 and 11% variance explained by the strata and 8 variables having from 45% to 87% variance explained.\JEP{Add the proportion of variance for each covariate as a column in Table 1. Reference Table 1 here.} 

## $B$-index calculations {#B-index}
...
## Standardized Mean Differences {#SMD-results}

Figures \@ref(fig:fig-smd-group1), \@ref(fig:fig-smd-group2), and \@ref(fig:fig-smd-group3) display individual covariate balance as measured by standardized mean differences. Each group of plots correspond to the three patterns on performance described in the paper.\JEP{Add these plots.}

(ref:fig-smd-group1-caption) Group 1: Sampling methods performed as expected. All stratified methods outperformed non-stratified methods. Better balance was attained by balanced sampling followed by random sampling and convenience sampling.

```{r fig-smd-group1, fig.cap = "(ref:fig-smd-group1-caption)"}
...
(ref:fig-smd-group2-caption) Group 2: Sampling methods equally well resulting in good balance.

```{r fig-smd-group2, fig.cap = "(ref:fig-smd-group2-caption)"}
# figs$smd.neutral %>%
#   apa_style_plot()
```


(ref:fig-smd-group3-caption) Group 3: Stratification sometimes resulted in poorer balance. 

```{r fig-smd-group3, fig.cap = "(ref:fig-smd-group3-caption)"}
# figs$smd.bad %>%
#   apa_style_plot()
```
...
## Sampling Feasibility {#feasibility-results}
 
Figure \@ref(fig:fig-rel-per) compares each sampling method to a reference method by plotting the factor of increased difficulty, calculated as the number of schools contacted by comparison method divided by the number of schools contacted by reference method. This gives us another perspective on the relative difficulty of each method. The straight horizontal line represents the reference method.\JEP{Modify this figure to show the other selection factors too.}\JEP{Use bubble labels in the plots and remove the figure legend.}

(ref:fig-rel-per-caption) Relative sampling difficulty of each method compared to URS. The the dashed horizontal line indicates equal difficulty (factor of 1).
