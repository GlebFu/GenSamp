...
load("Data/Paper/Analysis Figures 02-08-2021.rdata")
# figs <- list()
...
\end{align}
where $\bar{x}_p$ is the sample mean of covariate $p$, $\mu_p$ is the covariate mean in the population, and $\sigma_p$ is the standard deviation of the covariate in the population. $SMD$ values closer to zero indicate greater balance between the sample and the population. By this measure, obtaining a $SMD_p \le 0.25$ for all covariates in $P$ would indicate that the sample is well balanced and can be generalized to the population. Covariates for which $SMD_p > 0.25$ would likely require additional statistical adjustment in the final analysis [@tipton_review_2018].
...
tabs$pop.desc %>%
  select(Variable = Variables, M, SD, Log_odds = SF100, vratio) %>%
  kable(
    booktabs = T,
    caption = "Descriptive statistics for the population of 9,792 schools, assumed log odds of participation associated with each covariate, and proportion of variance explained in each covariate by stratification",
    col.names = c("Variables", "M", "SD", "Log-odds", "CVE")
  ) %>%
  kable_styling(
  ...
  add_footnote(label = "Log-odds = Log-odds of participation in data generating model",
               notation = "none") %>%
  add_footnote(label = "CVE = Cluster variance explained by stratification",
               notation = "none")
...
Figure \@ref(fig:fig-avg-B) displays the average $B$-index resulting from each recruitment method, for varying participation rates (on the horizontal axis) and selection factors. Acceptable values of $B$ for generalizability vary depending on the size of the sample, the size of the population, and the number of covariates [@tiptonHowGeneralizableYour2014]. Given our design, a value of $B = .95$ would indicate very good overlap with no need to adjust estimates. Generally $B >= .80$ still supports generalizability with adjustment and likely no need to redefine the population. 
...
```{r fig-SMD-groups, fig.height = 5, fig.width = 8, fig.cap = "(ref:fig-SMD-groups-caption)"}
...
```{r fig-Var-Coef, fig.width = 8, fig.height = 4, fig.cap = "(ref:fig-Var-Coef-caption)"}
# 
figs$v.coef +
  theme_apa() +
...
(ref:fig-responses-caption) Sample recruitment statistics and response rates for the highest selection factor $(SF = 1)$. (a) Total number of sites contacted to achieve a full sample of 60 sites. (b) Percentage of contacted sites that agreed to participate.

```{r fig-responses, fig.height = 6, fig.width = 8, fig.cap = "(ref:fig-responses-caption)"}

tfun <- function(x, 
                 # y.title = element_blank(),
...
responses.fig.list <- list(figs$samples.contacts %>% tfun(),
                           figs$samples.response.rates %>% tfun())
...
(ref:fig-smd-group1-caption) Group 1: Covariates where all methods consistently achieved good balance.
...
(ref:fig-smd-group2-caption) Group 2: Covariates where stratified sampling methods consistently achieved good balance, but where unstratified methods sometimes resulted in poor balance.
...
(ref:fig-smd-group3-caption) Group 3: Covariates where good balance was consistently achieved for SBS only, and where other stratified methods outperformed the unstratified counterparts 
...
(ref:fig-smd-group4-caption) Group 4: Covariates exhibiting unique patterns that did not match groups 1, 2 and 3. Also includes covariates where manipulating scale factor altered the pattern

```{r fig-smd-group4, fig.cap = "(ref:fig-smd-group4-caption)"}
figs$smd.appx.4 %>%
  apa_style_plot()
```
