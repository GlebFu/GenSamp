...
\end{align}
where $\bar{x}_p$ is the sample mean of covariate $p$, $\mu_p$ is the covariate mean in the population, and $\sigma_p$ is the standard deviation of the covariate in the population. $SMD$ values closer to zero indicate greater balance between the sample and the population. By this measure, obtaining a $SMD_p \le .25$ for all covariates in $P$ would indicate that the sample is well balanced and can be generalized to the population. Covariates for which $SMD_p > .25$ would likely require additional statistical adjustment in the final analysis. \GF{Should we foreshadow the B-Index here?}
...
Second, we assume that schools' response decisions are related to their measured covariates, $\mathbf{x}_j$, but not to unmeasured characteristics.\JEP{Any further comments about this assumption?}
...
To model stratified sampling methods, we will assume that the population is divided into a set of $K$ strata. Proportional allocation will be used to ensure the relative size of the strata in the sample reflects that of the population [@tiptonStratifiedSamplingUsing2013]. 
...
Our cluster analysis generally followed the approach and recommendations described by @tiptonStratifiedSamplingUsing2013. Specifically, we implemented k-medoids clustering to divide the population into heterogeneous strata comprised of homogeneous sites. We used Gower's general similarity measure, which is appropriate for covariates that include both continuous and categorical variables [@gowerGeneralCoefficientSimilarity1971; @tiptonStratifiedSamplingUsing2013].
...
```{r fig-avg-B, fig.pos="bth", fig.cap = "(ref:fig-avg-B-caption)"}
...
(ref:fig-SMD-groups-caption) Patterns of relative performance based on standardized mean differences. The dotted horizontal line represents a threshold of 0.25 for acceptable balance.

```{r fig-SMD-groups, fig.height = 5, fig.width = 7, fig.pos = "tb", fig.cap = "(ref:fig-SMD-groups-caption)"}



figs$smd.examples <- tabs$smd.results %>%
  filter(vnames %in% c("n", "pELL", "T1")) %>%
  nest(smd.plot.data = -c(Variables, vnames)) %>%
  mutate(plots = pmap(.l = list(data = smd.plot.data), .f = plot_smd, l.pos = "bottom"),
...
smd.example.list <-
  list(tfun(figs$smd.examples$plots[[1]]), 
       tfun(figs$smd.examples$plots[[2]]), 
       tfun(figs$smd.examples$plots[[3]]))


smd.example.legend <- (smd.example.list[[1]] + 
  theme_update(legend.box.margin = margin(0, 0, 0, 0),
               legend.position = "right")) %>% 
  get_legend()

smd.fig.list <- plot_grid(plotlist = smd.example.list,
                          ...
                          label_size = 10) + 
  theme(plot.margin = margin(0, 0, 24, 24))


plot_grid(smd.fig.list, 
          smd.example.legend, 
          rel_widths = c(3, .4)) +
...
figs$smd.examples2 <- tabs$smd.results %>%
  filter(vnames %in% vars$smd.examples ) %>%
  mutate(Group = factor(vnames, labels = c("Group 2", "Group 3", "Group 4", "Group 1"))) %>% 
    ...
    labs(y = lab.vars$smd,
         x = lab.vars$rr) + 
    scale_x_continuous(breaks = seq(10, 90, 40),
                       limits = c(10, 100)) +
    scale_alpha_continuous(guide = "none", range = c(.5, 1)) +
    facet_grid(Group ~ SB, scales = "free_y")

figs$smd.examples2 <- figs$smd.examples2 %>% 
  apa_style_plot() +
  theme(legend.position = "none")

plot_grid(figs$smd.examples2, smd.example.legend, rel_widths = c(3, .5)) +
  draw_label(lab.vars$rr,
             x= 0.5,
             y = 0,
             vjust = -0.25,
