...
Rather than relying on retrospective adjustments, a series of recent papers instead advocate designing robust sampling methods that focus on generalizability at the recruitment stage [@tiptonImprovingGeneralizationsExperiments2013; @tiptonStratifiedSamplingUsing2014]. Like retrospective adjustment, these methods also require a well-defined and enumerated population for which there is extant data.
...
Second, we assume that schools' response decisions are related to their measured covariates, $\mathbf{x}_j$, but not to unmeasured characteristics. Specifically, the decision to participate can be driven by school level characteristics (i.e., urbanicity, school size) or aggregated student data (i.e., student demographics, percent of students with IEPs). 
  \JEP{Any further comments about this assumption?}\GF{Added}
...
\end{equation}
Under this assumption, $E_j$ can be interpreted as an indicator for _potential response_, or whether a school would participate in the study _if approached for recruitment_, for any school in the population. 
This simplifying assumption could fail to hold if the likelihood that a school responds depends on the manner of recruitment.[^recruitment]

[^recruitment]: For example, a school might be more likely to respond to a recruitment attempt made through a face-to-face meeting than one from an email blast.
...
We know, for instance, that in many cases districts act as gatekeepers by determining whether or not to approve research proposals.
  \JEP{Cite?}\GF{This is sort of just known. I don't have a specific citation. I can confirm that almost all of the 15 largest districts in TX require submission to a district review board, for instance} 
...
While our results will not support inferences about these behaviors, they will nevertheless allow us to compare the performance of sampling methods and how they handle realistic challenges such as non-response.
  \JEP{Discuss difficulty of estimating this model from extant data, due to non-representative samples. Also how will we develop assumptions about values of $\boldsymbol\beta$?}
  \GF{added}
  \JEP{Hrmm, any way to improve the rhetoric here?}
  \GF{Added last two sentences}
...
Following @tiptonStratifiedSamplingUsing2014, we operationalize balanced sampling by prioritizing the recruitment of sites based on their similarity to the "average" site in each stratum. 
...
\JEP{Is this consistent with the Generalizer/Tipton 2013 approach?}
\GF{Tipton 2014b recommends this method if relationship between variables and participation is unknown or setting weights to 1 so that covariates with more variance drive stratification. Tipton + Olsen 2018 reccomend the latter strategy or using some proporotion of importance}
...
Scores within strata are based on equation \@ref(eq:euclid), where schools closer to the "center" of the stratum are more representative and are therefore a higher priority. Though extremely unlikely, it is possible that several schools could be equidistant from the center of the stratum; in such cases, schools are ordered randomly. Because @tiptonStratifiedSamplingUsing2014 proposed balanced sampling in connection with stratification based on a cluster analysis, we only consider the stratified version, SBS.
...
We study the performance of different recruitment methods for a study aimed at generalizing to the population of schools across a diverse set of six states: California, Oregon, Pennsylvania, South Carolina, Texas, and Wyoming. These six states were selected because they provided ready access to school- and district-level achievement data, which can be used to expand the current research.\JEP{Weird justification}\GF{Leaving for now} For illustrative purposes, we selected as covariates school characteristics that previous research has found to be associated with participation in RCTs. The goal is for the sample to include schools not normally found in large-scale evaluations of interventions [cf. @stuartCharacteristicsSchoolDistricts2017; @fellersDevelopingApproachDetermine2017; @tiptonSiteSelectionExperiments2016]. These studies found that districts and schools with higher proportions of students who are English language learners (ELL), economically disadvantaged (ED), non-White, and living in urban settings are more likely to participate.
...
tabs$pop.desc %>%
  ...
  add_footnote(label = "Note: SF is the scale factor used to manipulate the strength of the relationship between covariates and participation when multiplied by the originally reported log-odds. SF of 1, .5, and .25 are displayed. SF = .01 was also used which resulted in values near zero (< .001) and are therefore not displayed.",
               notation = "none")
...
In order to simulate the full process of recruiting a sample for an MRT, we need to choose parameter values for the participation propensity score model (Equation \@ref(eq:RGM)).
...
These values lead to large variation in the response propensity scores. In order to examine the extent to which our results are driven by this assumption, we considered three further conditions, in which the propensity model coefficients were divided by a scaling factor of .5, .25, and .01.
...
It can be seen that Group 1 consisted of covariates where a substantial fraction of the variance was between clusters. Groups 2 and 3 both consisted of covariates that were poorly clustered, with no more than 15% of variance between clusters. The major difference between groups 2 and 3 appears to be the relationship between the covariates and the likelihood of participation.\JEP{Further explain each group. How does the plot explain the performance of stratified methods for each set of covariates?}
...
# responses.fig.list <- list(figs$samples.contacts %>% 
...
A second goal was to use this framework to investigate the relative perfomance of different several sampling methods in terms of generalizability and feasibility. Based on these simulation, we draw several tentative conclusions. Stratified balanced sampling as proposed by @tiptonStratifiedSamplingUsing2014 has the potential to greatly increase the generalizability of samples selected for MRTs. However, this method is not without limitations. Within our simulation, strict application of SBS came at a great cost in terms of the number of schools that needed to be contacted. Thus, implementing this method in practice may require allocating many more study resources to sample recruitment.
...
(ref:fig-smd-group2-caption) Group 2: Sampling methods equally well resultiing in good balance.
