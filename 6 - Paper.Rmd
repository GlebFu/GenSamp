...
```{r fig-SMD-by-Var-bad, fig.pos = "tb", fig.width = 9, fig.height = 3, fig.cap = "Group 4: poor relative performance for stratified convenience sampling and stratified random sampling. Dotted horizontal line indicates a .25 cutoff."}
...
plt.feas$count <- figs$samples.contacts + 
  (if (grey.plots) {scale_colour_grey(start = 0, end = .7)}) +
  theme_apa(box = F) +
  theme(legend.position = "none") 
  
plt.feas$rate <- figs$samples.response.rates + 
  (if (grey.plots) {scale_colour_grey(start = 0, end = .7)}) +
  theme_apa(box = F) +
  theme(legend.position = "none") 

legend <- get_legend(
  plt.feas$count + theme(legend.position = "right") 
)
