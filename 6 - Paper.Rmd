...
(ref:fig-avg-B-caption) Average $B$-index for varying participation rates across SFs, by sampling method. Horizontal dotted and dashed lines represented indices of .80 and .95, respectively, indicating acceptable or high levels of generalizability.


```{r fig-avg-B, fig.height = 4, fig.width = 8, fig.pos="bth", fig.cap = "(ref:fig-avg-B-caption)"}
...
(ref:fig-SMD-groups-caption) Patterns of relative performance based on standardized mean differences. The dotted horizontal line represents a threshold of 0.25 for acceptable balance. Results for the highest level of selection bias (SF = 1) is displayed to highlight differences in groups.

```{r fig-SMD-groups, fig.height = 5, fig.width = 7, fig.pos = "tb", fig.cap = "(ref:fig-SMD-groups-caption)"}
...
```{r fig-Var-Coef, fig.cap = "(ref:fig-Var-Coef-caption)", fig.pos="tb"}
...
plot_grid(plotlist = responses.fig.list,
          ...
          label_size = 10,
          align = "v") %>%
  plot_grid(get_legend(figs$samples.contacts.sf100 %>% apa_style_plot()),
            rel_widths = c(3, .4)) +
...
A further pattern in these simulation results relates the specification of the clustering method to the response model. Covariates that are highly related to determining participation must be prioritized in generating the strata. If such covariates are poorly allocated across strata (i.e., there is little heterogeneity between strata) then the purpose of stratification is undercut and it will not result in good balance on the individual covariate. It is therefore important to not only examine overall variance explained when performing the cluster analysis, but to use individual covariate variability as a criterion as well. \JEP{Wonder if this isn't more of a discussion point (because it's drawing implications, rather than just summarizing.)}
...
A second goal was to use this framework to investigate the relative performance of different several sampling methods in terms of generalizability and feasibility. Based on these simulation, we draw several tentative conclusions. Stratified balanced sampling as proposed by @tiptonStratifiedSamplingUsing2014 has the potential to greatly increase the generalizability of samples selected for multi-site randomized trials. However, this method is not without limitations. Within our simulation, strict application of SBS came at a great cost in terms of the number of schools that needed to be contacted. Thus, implementing this method in practice may require allocating many more study resources to sample recruitment.
...
Selecting an appropriate number of clusters is one of the most difficult problems in cluster analysis [@steinleyKmeansClusteringHalfcentury2006]. @tiptonStratifiedSamplingUsing2014 argued that both empirical and practical criteria should be used in selecting $K$. @hennigHowFindAppropriate2013 also argued that the method of selecting $K$ should depend on the context of the clustering, framing the issue as one of obtaining an appropriate subject-matter-dependent definition rather than a purely statistical question. 
...
```{r fig-k-plots, fig.height = 7, fig.width = 8, out.width = "1\\linewidth", fig.pos = "tbp", fig.cap = "(ref:fig-k-plots-caption) "}
