...
# params <- list(
#   iterations = 2,
#   RR.condition = unique(df.PS$RR)[1],
#   K.condition = c("K_05"),
#   SB.condition = unique(df.PS$scale_factor)[1]
# )
...
```{r}
test$df.recruitment.stats[[1]] %>% group_by(sample_method, strata) %>% count

test$df.samp.counts[[1]] %>% group_by(sample_method) %>% count


test$df.recruitment.stats[[1]] %>% group_by(sample_method) %>% count
