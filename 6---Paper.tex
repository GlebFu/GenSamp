...
\todo[color=blue!20]{Any further comments about this assumption?}\todo[color=orange]{Added}
...
\todo[color=blue!20]{Cite?}\todo[color=orange]{This is sort of just known. I don't have a specific citation. I can confirm that almost all of the 15 largest districts in TX require submission to a district review board, for instance}
...
\todo[color=blue!20]{Discuss difficulty of estimating this model from extant data, due to non-representative samples. Also how will we develop assumptions about values of $\boldsymbol\beta$?}
\todo[color=orange]{added}
\todo[color=blue!20]{Hrmm, any way to improve the rhetoric here?}
\todo[color=orange]{Added last two sentences}
...
\todo[color=blue!20]{Is this consistent with the Generalizer/Tipton 2013 approach?}
\todo[color=orange]{Tipton 2014b recommends this method if relationship between variables and participation is unknown or setting weights to 1 so that covariates with more variance drive stratification. Tipton + Olsen 2018 reccomend the latter strategy or using some proporotion of importance}
...
\bottomrule
\multicolumn{6}{l}{\textsuperscript{} Note: SF is the scale factor used to manipulate the strength of the relationship between}\\
\multicolumn{6}{l}{covariates and participation when multiplied by the originally reported log-odds. SF of}\\
\multicolumn{6}{l}{1, .5, and .25 are displayed. SF = .01 was also used which resulted in values near zero}\\
\multicolumn{6}{l}{(< .001) and are therefore not displayed.}\\
\end{tabular}
\end{table}
