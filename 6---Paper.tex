...
\affiliation{
\vspace{0.5cm}
\textsuperscript{1} University of Texas at Austin\\\textsuperscript{2} University of Wisconsin-Madison}
...
\end{align}
where \(w_p\) is a weight assigned to covariate \(X_p\).\todo[color=blue!20]{Why use Euclidean here but Gower below?} \todo[color=orange]{This is how it was presented in Tipton 2013. Urbanicity was dummy coded} Different weights could be used such that distances depend more heavily on covariates thought to be more related to treatment effect heterogeneity. To weigh covariates equally, we set the weight weight of a covariate to the inverse of its population variance as \(w_p = 1/\sigma^2_p\).
...
We calculated the index by first estimating a population propensity score.\todo[color=blue!20]{Details about estimated propensity score.}
...
\end{align}
where \(\hat\psi_{min}\) and \(\hat\psi_{max}\) are \todo[color=blue!20]{define}, \(\hat{f}_s(z)\) is a Gaussian kernel density estimate of the distribution of propensity scores in the sample, calculated using bandwidth \(h_s\), and \(\hat{f}_p(z)\) is a Gaussian kernel density estimate of the distribution of propensity scores in the non-sampled population, calculated using bandwidth \(h_p\). We calculated the bandwidth \(h\) for the population or sample as follows:
\begin{align}
h = \sigma_{\pi}\left(\frac{4}{3n}\right)^{1/5}
\end{align}
where \(\sigma_{\pi}\) is the standard deviation of the propensity scores. \todo[color=orange]{Do these equations look right?}\todo[color=blue!20]{Could you point me to the lines in the code?}
...
Imai, K., King, G., \& Stuart, E. A. (2008). Misunderstandings between experimentalists and observationalists about causal inference. \emph{Journal of the Royal Statistical Society: Series A (Statistics in Society)}, \emph{171}(2), 481--502. doi:\href{https://doi.org/10.1111/j.1467-985X.2007.00527.x}{10.1111/j.1467-985X.2007.00527.x}
...
Tipton, E., Spybrook, J., Fitzgerald, K. G., Wang, Q., \& Davidson, C. (2020). Toward a System of Evidence for All: Current Practices and Future Opportunities in 37 Randomized Trials. \emph{Educational Researcher}, 0013189X20960686. doi:\href{https://doi.org/10.3102/0013189X20960686}{10.3102/0013189X20960686}
