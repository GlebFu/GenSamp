---
title: "Untitled"
output: pdf_document
---

# Setup

## Packages and Data

```{r}
library(papaja)
library(tidyverse)
library(kableExtra)
library(ggalt)
library(ggrepel)
library(ggforce)
library(concaveman)
library(cowplot)

rm(list = ls())


load("Data/Simulation Data/Sim Data.Rdata")
load("Data/Cluster Analysis/Clusters k2-10.rdata")


# load(read_file("Data/Results/LastResults.txt"))
load("Data/Results/20201211 - 12 Reps - K5.rdata")


```


```{r}
ls() %>%
  tibble(Vars = .) %>%
  mutate(MB = map(Vars, function(x) object.size(eval(parse(text = x))) %>% format(units = "MB", digits = 3))) %>%
  unnest(MB) %>%
  arrange(desc(parse_number(MB)))

```

```{r}

df.list <- list()
results <- list()
vars <- list()

tabs <- list()
figs <- list()

df.list$strata.id <- df.clusters %>% 
  transmute(DSID = DSID, 
            k = K_05, 
            strata = factor(paste("Strata", k)))

df.list$pop.data <- left_join(df.sim, df.list$strata.id)



df.list$PS <- df.PS %>% 
  mutate(logit = log(PS / (1 - PS)))

df.list$cluster.vars <- read_csv("Data/Paper/Clustering Variables.csv")

df.list$coefs <- DGM.Bs %>%
  transmute(vnames = var, log_odds = Bs) %>%
  full_join(read_csv("Data/Paper/Clustering Variables.csv"))

df.list$pop.stats <- df.list$pop.data %>%
  select(DSID, all_of(covariates), strata) %>%
  gather(key = vnames, value = value, covariates) %>%
  group_by(vnames) %>%
  mutate(T.SS = var(value) * (n() - 1)) %>%
  group_by(vnames, T.SS, strata) %>%
  summarise(W.SS = var(value) * (n() - 1)) %>%
  summarise(W.SS = sum(W.SS)) %>%
  mutate(B.SS = T.SS - W.SS,
         vratio = B.SS / T.SS) %>%
  left_join(df.list$coefs) %>%
  left_join(df.pop.stats %>% rename(vnames = var))
```

```{r}
rename_results <- function(result.data) {
  result.data %>%
  separate(sample_method, into = c("Cluster", "Sample", "Rank_Type"), remove = F) %>%
  rename(Sampler = sample_method) %>%
  mutate(K = parse_number(K),
         RR = parse_number(RR))
}


df.list$k.stats <- cluster.stats

results$B <- df.B.indicies %>% rename_results()

results$r.stats <- df.recruitment.stats %>% 
  rename_results() %>%
  group_by(K, Sampler, Cluster, Sample, Rank_Type, RR, SB, strata) %>%
  summarise(contacted = mean(sch.contacted),
            accepted = mean(sch.contacted))




```


```{r}
# results$gini.coef <- df.samp.counts %>%
#   rename_results() %>%
#   group_by(Sampler, Cluster, Sample, Rank_Type, DSID, RR, SB) %>%
#   count(name = "count") %>%
#   group_by(Sampler, Cluster, Sample, Rank_Type, RR, SB) %>%
#   nest() %>%
#   mutate(data = map(data, full_join, df.list$pop.data %>% select(DSID))) %>%
#   unnest(cols = c(data)) %>%
#   group_by(Sampler, Cluster, Sample, Rank_Type, RR, SB) %>%
#   mutate(count = ifelse(is.na(count), 0, count),
#          prop.selected = count/sum(count),
#          n = n(),
#          prop.pop = 1/n()) %>%
#   group_by(Sampler, Cluster, Sample, Rank_Type, RR, SB) %>%
#   arrange(prop.selected) %>%
#   mutate(c.prop.sel = cumsum(prop.selected),
#          c.prop.pop = cumsum(prop.pop)) %>%
#   summarise(gini = ineq::Gini(prop.selected) %>% round(2)) %>%
#   group_by(RR) %>%
#   arrange(gini) %>%
#   mutate(y.pos = 1 - (1:n()/n()) / 1.5,
#          x.pos = 1 - y.pos)



```

```{r}

results$smd.stats <- df.samp.stats %>% 
  rename(vnames = var) %>%
  ungroup() %>% 
  rename_results() %>% 
  select(-K) %>% 
  group_by(Sampler, Cluster, Sample, Rank_Type, RR, SB, vnames, strata) %>%
  summarise(sim.mean = mean(samp.mean)) %>%
  left_join(df.list$pop.stats) %>%
  mutate(mSMD = (sim.mean - pop.mean) / pop.sd)


```


```{r}

results$k <- df.clusters

# df.list$coefs <- data.frame(vnames = names(DGM.Bs), 
#                             log_odds = DGM.Bs, 
#                             row.names = NULL) %>%
#   full_join(read_csv("Data/Paper/Clustering Variables.csv"))

rm(list = c("cluster.stats", 
            "df.B.indicies", 
            "df.recruitment.stats", 
            "df.samp.counts", 
            "df.samp.stats", 
            "df.clusters", 
            "df.pop.stats", 
            "df.PS",
            "rename_results"))

vars$cluster <- list(mixed = cluster_vars, dummy = covariates)


```

```{r}
ls() %>%
  tibble(Vars = .) %>%
  mutate(MB = map(Vars, function(x) object.size(eval(parse(text = x))) %>% format(units = "MB", digits = 3))) %>%
  unnest(MB) %>%
  arrange(desc(parse_number(MB)))


save.image(file = "Data/Paper/Summarised Sim Results.RData")


```