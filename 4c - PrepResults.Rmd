...
df.list$pop.cluster.stats <- df.list$pop.data %>%
  select(DSID, all_of(covariates), strata) %>%
  bind_rows(mutate(., strata = "Strata 0")) %>%
  gather(key = vnames, value = value, covariates) %>%
  group_by(vnames, strata) %>%
  summarise(pop.mean = mean(value),
            pop.sd = sd(value)) %>%
...
results$smd.stats <- sim.results %>% 
  ...
  select(-pop.mean, -pop.sd) %>%
  left_join(df.list$pop.cluster.stats) %>%
