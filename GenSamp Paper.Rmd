...
### Number of Strata
Next we used the _kmeans_ function to generate clusters, which uses an optimization algorithm to classify sites into $k$ clusters by minimizing the total within cluster sum of squares. For each $k$, it is recommended to run _kmeans_ at least 10 times, and select the clustering that results in the smallest total within-cluster sum of squares. This function also requires two parameters: (1) the distance matrix (generated in the previous step), and (2) the number of clusters to generate ($k$). 
...
(ref:fig-avg-Bindex-caption) Averge $B$-index for varying participation rates, by sampling method
