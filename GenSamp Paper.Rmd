...
plt.k$ch <- cluster.stats %>%
  ggplot(aes(x = K, y = ch)) +
  geom_point() +
  geom_line() +
  theme_apa() +
  scale_x_continuous(limits = c(minK,10),
                     breaks = c(minK:10)) +
  labs(y = "CH index",
       x = "")
...
plt.k$vrat <- cluster.stats %>%
  mutate(min80 = sum(bt.ss < .8) - .5 + minK) %>%
...
plt.good1 <- c("ethHisp", "pELL", "Urban", "ethWhite", "ToRu")
plt.good2 <- c("pTotfrl", "T1",  "Suburban")
plt.bad2 <- c("dSCH")
...
tabs$ICC.vs.Coefs <- df.icc %>%
  ...
  mutate(icc = round(icc, 3),
         ...
         results = ifelse(vnames %in% plt.bad2, "Group 5", results),
...
```{r fig-SMD-by-Var-bad, fig.pos = "tb", fig.width = 9, fig.height = 3, fig.cap = "Groups 4 and 5: poor relative performance for stratified convenience sampling and stratified random sampling. Dotted horizontal line indicates a .25 cutoff."}
...
sSMDplot %>%
  ...
  ungroup() %>%
  mutate(RR = parse_number(RR)) %>%
  filter(var %in% c(plt.bad, plt.bad2)) %>%
  ggplot(aes(x = RR, y = mSMD, color = Sampling, linetype = Clustering, group = sample_method)) +
  # geom_point() +
  geom_line() +
  geom_hline(yintercept = .25, linetype = "dotted") +
...
df.var.ratio %>%
  ggplot(aes(x = vratio, y = log_odds, label = Variables, color = results, fill = results)) +
  geom_point(size = 3) +
  geom_encircle(alpha = .2, s_shape = 0, expand = 0.1, spread = .05, color = "black") +
  geom_text_repel(point.padding = .2, color = "black") +
