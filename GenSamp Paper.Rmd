...
```
  ...
  In absence of random sampling, PATE can still be estimated by using propensity score techniques, provided there is sufficent overlap between the sample and the population [@omuircheartaighGeneralizingUnrepresentativeExperiments2014;@tiptonImprovingGeneralizationsExperiments2013, @kernAssessingMethodsGeneralizing2016]. This is especially usefull in impact evaluation as probability sampling is rarely used in this context [@shadishExperimentalQuasiexperimentalDesigns2002; @olsenExternalValidityPolicy2013]. Instead, researchers often opt for convenience or purposive samples. These methods much less expensive to implement, but are not usually designed for representative sampling. This makes them susceptible to undercoverage, or substantial differencs between the sample and target population [@grovesSurveyMethodology2004]. 
  
  Undercoverage can be assessed using several techniques [@stuartUsePropensityScores2011; @tiptonHowGeneralizableYour2014] to identify how well a sample would generalize to a specific population. When undercoverage is too great, the full PATE cannot be recovered with the given sample, and the population needs to be trimmed by removing subsets of sites that are not represented. This can greatly diminish the relavence of study results and undermine the substantial investment into large-scale MRTs.
...
schVals %>%
  ...
  mutate(RR = paste(RR*100, "%", sep = "")) %>%
  na.omit() %>%
  ggplot(aes(x = RR, y = abs(dif), group = Var)) +
  geom_point() +
  geom_line() +
  theme_apa() +
  labs(x = "Population Response Rates",
