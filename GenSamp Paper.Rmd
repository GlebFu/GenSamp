...
# Introduction
  ...
  The multi-site randomized trial (MRT) has become a common design in educational research, particularly in effectiveness studies of educational interventions. In essence, an MRT is a randomized control trial (RCT) that takes place accross multiple distinct sites (e.g. geographic locations), with random assignment taking place either at the site or unit level. In educational research, an MRT may consist of multiple schools being recruited from a district, after which each school is randomly assigned to implement an intervention (treatment) or continue business as usual (control).
  
  The advantage of an MRT design over an RCT is that it maintains the high level of internal validity via randomized treatment assignment, while also supporting a greater degree of generalizability by introducing cross-site variation. Ostensibly, this allows effect estimates to generalize to a larger population than estimates from a single-site design [@raudenbushStatisticalPowerOptimal2000]. Recently, however, the ambiguity of how this larger population is defined has increasingly come under scrutiny. 
  
  Generalizability falls under the purview of external validity, and is a multi-faceted problem in causal inference. For the purpose of the present study, we define generalizability as the extent to which a sample is representative of a well-specified population. In this context generalizability is of particular interest in evidence-based policy decisions, where the primary concern is intervention effectiveness on units outside of the study sample.
  
  Inadequate sampling strategies are a major threat to generalizability of MRTs. Several studies have found substantial differences between schools and districts that participate in large scale randomized trials [@fellersDevelopingApproachDetermine2017; @stuartCharacteristicsSchoolDistricts2017]. Purposive or non random samples may lead to substantially biased estimates of population level effects [@olsenExternalValidityPolicy2013; @shadishExperimentalQuasiexperimentalDesigns2002]. Furthermore, if the sample of sites is too small or homogeneous, the scope of the generalization is greatly diminished.
