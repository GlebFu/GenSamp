...
Though there are very few demonstrations of SBS in the literature, several studies outline the method's benefits [@tiptonSiteSelectionExperiments2016; @tiptonStratifiedSamplingUsing2013], Generally, stratified sampling methods should offer several key advantages besides better generalizability. First, the method requires that researchers document the recruitment process, carefully consider the inference population, and justify targeting specific sites. This supports transparency and enables a more careful critique of the sampling design and study inferences. Second, careful documentation also supports follow up analysis on participation behavior such as systematic differences across non-responders. Finally, SBS easily integrates with post-hoc statistical adjustment techniques. Even if balance is only partially improved at the sampling stage, coverage errors will still be reduced and less of the inference population will need to be discarded during analysis.
...
(ref:fig-avg-Bindex-caption) Average $B$-index for varying participation rates, by sampling method. Horizontal dotted line represented index of .95 indicating a high level of generalizability.
...
(ref:fig-ICCvsCoef-caption) ICC Values vs absolute log odds. Shaded areas illustrate potential patterns in generalizability measured by SMDs.

```{r fig-ICCvsCoef, results = "asis", eval = T, fig.cap = "(ref:fig-ICCvsCoef-caption)", eval = T, warning = F, message = F}
library(ggalt)
library(ggrepel)
...
tabs$ICC.vs.Coefs %>%
  ...
  # geom_encircle(aes(fill = results) , alpha = .1, s_shape = 0, expand = 0.1, spread = .1) +
...
(ref:fig-SMD-by-Var-good1-caption) Good balance for stratified methods. Dotted horizontal line indicates a .25 cutoff.

```{r fig-SMD-by-Var-good1, results = "asis", eval = T, fig.cap = "(ref:fig-SMD-by-Var-good1-caption)", eval = T, warning = F, message = F}
...
(ref:fig-SMD-by-Var-neutral-caption) All methods performed well. Dotted horizontal line indicates a .25 cutoff.

```{r fig-SMD-by-Var-neutral, results = "asis", eval = T, fig.cap = "(ref:fig-SMD-by-Var-neutral-caption)", eval = T, warning = F, message = F}
...
(ref:fig-SMD-by-Var-good2-caption) Good balance for stratified methods. Dotted horizontal line indicates a .25 cutoff.

```{r fig-SMD-by-Var-good2, results = "asis", eval = T, fig.cap = "(ref:fig-SMD-by-Var-good2-caption)", eval = T, warning = F, message = F}
...
(ref:fig-SMD-by-Var-bad-caption) Poor balance for stratified methods. Dotted horizontal line indicates a .25 cutoff.
...
(ref:fig-responses-caption) Sampling statistics. The top plot shows the total number of units contacted to achieve a full sample of 60 schools. The bottom plot shows the percent of schools that agreed to participate when recruited.

```{r fig-responses, results = "asis", fig.cap = "(ref:fig-responses-caption)", eval = T}
tab.feas <- samplot %>%
  ungroup() %>%
  mutate(RR = parse_number(RR),
...
(ref:fig-comp-caption) Proportionate comparisons of difficulties between models. The straight horizontal line is the focus of comparison in each panel.

```{r fig-comp, results = "asis", fig.cap = "(ref:fig-comp-caption)", eval = T}
...
```{r fig-response-rates, results = "asis", fig.cap = "(ref:fig-response-rates-caption)", eval = F}
...
```{r fig-gini, results = "asis", fig.cap = "(ref:fig-gini-caption)", eval = T}
...
(ref:fig-gini-curve-caption) Gini plot and coefficient representing the inequality of school sampling across sampling methods and population response rates.

```{r fig-gini-curve, results = "asis", fig.cap = "(ref:fig-gini-curve-caption)", eval = T, fig.env = "sidewaysfigure", fig.height = 6, fig.width = 10}
df.gini.count %>%
  # filter(RR == "RR_90") %>%
