...
tabs$ICC.vs.Coefs %>%
  ggplot(aes(x = icc, y = log_odds, color = results)) +
  geom_point() +
  geom_label(aes(label = Variables))
...
(ref:fig-SMD-by-Var-good-caption) Good balance for stratfied methods

```{r fig-SMD-by-Var-bad, results = "asis", eval = T, fig.cap = "(ref:fig-SMD-by-Var-bad-caption)", eval = T, warning = F, message = F}
# plt.vgood <- c("dSCH", "ethWhite")
# plt.good <- c("ethHisp", "pELL", "pTotfrl", "T1", "Urban", "ToRu", "Suburban")
