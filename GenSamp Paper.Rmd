...
## Population
  ...
  We assume a data set enumerating a population of $N$ sites (schools), indexed as $i = 1 ... N$. Each site has some unobserved probability of participating if recruited represented by the participation propensity score $\pi_i$. Each site also has  a vector of observed characteristics $X_i$ of length $P$. This is a set of pre-treatment covariates that predict $\pi_i$. The covariates are a mixture of continuous, binary, and categorical data.
  
  Our goal is to select a sample of $n$ sites that is representative of the population in that it is balanced along the set of $P$ covariates. We measure balance using the standardized mean difference ($SMD$) between the sample and population for a given covariate. $SMD$ is calculated as 
...
To select parameters for the RGM we first identified desirable sample characteristics. @fellersDevelopingApproachDetermine2017 compared 571 elementary schools that participated in IES funded studies to the full population of U.S. elementary schools on a set of similar covariates, and reported the absolute SMD between the schools that participated and the population. These values served as our "goal" SMDs ($SMD^G$). Iterating across parameter values, we generated $\pi^S$ and computed the expected mean ($m^E$) from a random sample as
\begin{align}
...
```{r fig-SMD-goal, results = "asis", fig.cap = "Difference between expected SMDs and goal SMDs"}
schVals_lab <- schVals %>%
  filter(RR == .5) %>%
...
#   mutate(RR = paste(RR*100, "%", sep = "")) %>%
#   na.omit() %>%
#   ggplot(aes(x = RR, y = dif, group = Var, linetype = Var)) +
#   geom_point() +
#   geom_line() +
#   theme_apa() +
...
if(saveFigs) save.figures("Intro - Differences in SMDs")
...
#   select(Var, pars, RR) %>%
#   mutate(RR = paste(RR*100, "%", sep = "")) %>%
#   ggplot(aes(x = RR, y = pars, group = Var, linetype = Var)) +
#   geom_point() +
#   geom_line() +
#   theme_apa() +
#   labs(x = "Population Response Rates",
...
if(saveFigs) save.figures("Intro - Parameter Values")
```
...
load(paste("Paper Data/", file_date, "/Result Data.rdata", sep = ""))
...
sSMDplot$Variable <- as.factor(sSMDplot$Variable)
levels(sSMDplot$Variable) <- c("School Size", "Free/Reduced Lunch", "English Proficient", "ELL", "Math Proficient", "Non-White", "Suburban", "Town/Rural", "Urban")
...
# 
# if(saveFigs) save.figures("Results - SMD - School Size")
# 
