...
# Stratified Balanced Sampling
  In this section we demonstrate an SBS within the context of selecting schools for a large-scale MRT. This application will also serve as the basis for the present simulaion study which will be further outlined in a later section. SBS is a flexible method that is adaptble to various data sets, and may therefor require different strategies or methods than presented here depending on study circumstances and goals. The overview presented here is specific to the context we have previously outlined. For a more complete consideration of the various ways in which SBS can be applied, see @tiptonStratifiedSamplingUsing2013. 
  SBS applies cluster analysis as a dimension reduction technique to drive bias robust balanced sampling based on a large set of covariates. The ultimate goal is to select a sample that is representative of a population along a set of covariates related to treatment heterogeneity and site selection. This process requires the availability of a rich data set of observed covariates for each site in the population. We therefore begin by discussing the data which will serve as the sampling frame and the covariates whe have selected. 
  Following recomendations proposed by @tiptonStratifiedSamplingUsing2013, we then implement k-means clustering to divide the population into heterogeneous strata comprised of homogeneous sites. K-means clustering assigns sites to strata such that similarity within strata is maximized. This step requires us to specify the number of strata that need to be generated, and may rely on both empirical criteria, as well as subjective appraisals of what is feasible to implement.
  After determining the strata, we then use a second distance metric to rank sites in order of how representative the site is of its stratum. We then prioritize site recruitment based on these ranks. This step is referred to as "balanced sampling", and, in practice, should encourage the selection of sites from subsections of the population which may otherwise not have been included given typical sampling strategies.
...
## Population Frame
  We first identify a population of schools from which we want to select a representative sample. Six States served as the geographic boundries for  this population frame: California, Oregon, Pensilvania, South Carolina, Texas, and Wioming. These six states were selected for arbitrary reasons, blah blah blah.
  To perform the cluster analysis, the data must not only enumerate all population units, but also provide key characteristics that researchers hypothesize mediate treatment heterogeneity. Since we are only interested in selecting a representative sample, we instead identified covariates that predict school participation in RCTs. Our goal here is that our final sample will include schools not normaly found in large-scale evaluations of interventions. 
  
    Characteristics of the population frame were sourced from the Common Core of Data (CCD). The CCD is a comprehensive database housing annually collected statistics of all public schools and districts. Selection of covariates was driven by prior research on district and school participation behavior in RCTs (Stuart et al., 2017; Tipton et al., 2016a; Fellers, 2017). Districts and schools with higher proportions of students who are English language learners (ELL), economically disadvantaged (ED), non-White, and living in urban settings are more likely to participate, as are larger districts and schools. It is important to note, however, that some of these characteristics might also make it more likely that researchers would recruit these districts and schools in the first place. Therefore, the participation behavior outlined here is likely not representative of all schools in the US.
...
## Distance Metric
The distance metric is used to summarizes the difference between a pair of sites on a set of covariates in order to maximize the similarity of all sites within a clusters. It is therefore necessary to select a metric that is appropriate given the types of covaraites being used. In educational research, data are likely to contain both continuous and categorical data. For mixed data such as this it is appropriate to use the general similaity measure [@gowerGeneralCoefficientSimilarity1971; @tiptonStratifiedSamplingUsing2013].

This method relies on different calculations of distance depending on the type of covariates. Let $X_{hi}$ and $X_{hi'}$ be the observed value of covariate $h = {1, ..., P}$ for sites $i$ and $i'$ respectively, where $i \ne i'$. Let $d_{ii'h}$ be the distance between observed values of covariate $X_{h}$ for site $i$ and site $i'$. For categorical or dummy coded variables, $d_{ii'h} = 1$ if $X_{hi} = X_{hi'}$ and $d_{ii'h} = 0$ otherwise. For continuous covariates, we use the following formula:
...
## Selecting k Strata
Next we must determine the number of strata to be generated. A larger set of strata would result in greater homogeneity within each stratam, however in practical applications more strata may also be difficult to manage. For instance, if refusal and non-response rates are fairly high, having fewer sites spread across more strata may make it difficult to adequately recruit from all strata. Resource constraints (e.g. time, funding, recruiters) may also be a factor in the number of strata selected.
...
As $p_k$ approaches 1, most of the variation is between strata, indicating homogeneity within strata. This increases the possibility of selecting a more balanced sample. Plotting $p_k$ against $k$ allows visual comparison of the results. The $k$ for which the rate of change $p_k$ slows is considered favorable.
...
```{r fig-intercepts, results = "asis", fig.cap = "Intercepts used to generate response rates", eval = F}
