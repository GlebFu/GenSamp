...
knitr::opts_chunk$set(echo = F, cache = F, warnings = T, cache = T)
```
...
df.list <- list()
results <- list()
lab.vars <- list(k = "Number of strata (k)", vratio = "Variance Ratio (B/T)", ch = "CH Index")
...
df.list$coefs <- data.frame(Variables = names(DGM.Bs), vnames = names(DGM.Bs), log_odds = DGM.Bs, row.names = NULL) %>%
  arrange(Variables)

levels(df.list$coefs$Variables) <- c("District Size", 
                                "% Black",
                                "% Hispanic",
                                "% White",
                                "N Students",
...
figs$ch <- df.list$k.stats %>%
  ...
  geom_point() +
  geom_line() +
  scale_x_continuous(breaks = c(minK:K)) +
  scale_y_continuous(breaks = seq(4000, 9000, 1000),
                     limits = c(4000, 10000)) +
  labs(y = lab.vars$ch,
       x = lab.vars$k)
...
lab.vars$n_j <- "Target sample per cluster"
lab.vars$pop.prop <- "Proportion of population (%)"
...
lab.vars$smd <- "Standardized mean difference"
lab.vars$vars <- "Participation covariates"
...
lab.vars$intercept <- "Participation model intercept"
lab.vars$rr <- "Population participation \n(generated %)"
...
vars$plot.groups <- bind_rows(
  tibble(group = "Group 1", 
         desc = "good1", 
         vnames = c("ethHisp", "pELL", "Urban", "ethWhite", "ToRu")),
  tibble(group = "Group 2", 
         desc = "good2", 
         vnames = c("pTotfrl", "T1",  "Suburban")),
  tibble(group = "Group 3", 
         desc = "bad", 
         vnames = c("ethBlack", "n", "dSCH")),
  tibble(group = "Group 4", 
         desc = "neutral", 
...
lab.vars$coef <- "Participation model coeficients \n(absolute log-odds)"
...
figs$v.coef <- tabs$v.coef  %>%
  ggplot(aes(x = vratio, y = log_odds, color = group, fill = group)) + 
...
lab.vars$contacts <- "Number of schools contacted"
...
lab.vars$rr.o <- "Population participation  \n(observed %)"
...
# figs$counts.data <- results$r.counts %>%
#   group_by_all() %>%
#   count(name = "count") %>%
#   ungroup() %>%
...
# 
# figs$counts.pop.data <- df.list$PS %>%
#   left_join(df.list$strata.id) %>%
#   left_join(df.list$pop.data)
# 
# figs$counts.data  %>%
#   ggplot() +
#   geom_density(aes(x = PS, fill = sample_method, weight = w), alpha = .1) +
#   geom_density(data = figs$counts.pop.data ,
#                aes(x = PS)) +
...
figs$gini.coef <- tabs$gini.count %>%
  ...
  geom_label(aes(label = gini, x = x.pos, y = y.pos), data = tabs$gini.coef) +
  facet_wrap(~RR) +
  theme_apa()
...
#      df.icc,
#      df.gini.count,
#      file = "Data/Paper/Paper Data.rdata")
```