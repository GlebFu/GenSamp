...
```{r}
df.samp.counts %>%
  group_by(RR, sample_method, DSID) %>%
  summarise(count = n()) %>%
  ggplot(aes(sample_method, count)) +
  geom_boxplot() +
  facet_wrap(~ RR) +
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 45, hjust = 1))
...
test %>%
  filter(RR == "RR_30") %>%
  mutate(sample_method = str_replace(sample_method, "tratified", "")) %>%
  group_by(RR, strata, sample_method) %>%
  mutate(wt = count / sum(count)) %>%
  ggplot() +
  geom_density(aes(x = PS, fill = sample_method, weight = wt), alpha = 0.2) +
  geom_density(data = test2, aes(x = PS), size = .5) + 
  facet_grid(sample_method ~ strata, scales = "free_y") + 
  theme_minimal() + 
  theme(legend.position = "none")
...
test %>%
  ...
  mutate(wt = count / sum(count)) %>%
  ggplot() +
  geom_density(aes(x = ethHisp, fill = sample_method, weight = wt), alpha = 0.2) +
  geom_density(data = test2, aes(x = ethHisp), size = .5) + 
  facet_grid(sample_method ~ strata, scales = "free_y") + 
  theme_minimal() + 
  theme(legend.position = "none")
...
df.samp.counts %>%
  group_by(RR, sample_method) %>%
  summarise(excluded = sum(df$DSID %in% DSID))

# df.samp.char <- df.sim %>% 
#   select(DSID, covariates) %>%
#   mutate_if(function(x) !is.character(x), function(x) (x - mean(x)) / sd(x)) %>%
#   right_join(df.samp.counts)
# 
# df.samp.char <- df.PS %>%
#   right_join(df.samp.char) %>%
#   left_join(df.clusters)
