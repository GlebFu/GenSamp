...
rm(list = c("cluster.stats", "df.B.indicies", "df.recruitment.stats", "df.samp.counts", "df.samp.stats"))
```
...
tabs$cluster.SMD %>%
  ...
  ggplot(aes(x = SMD, y = var, color = strata, alpha = sig, label = k)) +
  geom_text() +
...
levels(df.coefs$Variables) <- c("District Size", 
                                "% Black",
                                "% Hispanic",
                                "% White",
                                "N Students",
...
df.gini.count %>%
  ...
  geom_label(aes(label = gini, x = x.pos, y = y.pos), data = df.gini.coef) +
  facet_wrap(~RR) +
  theme_apa()
```
...
```{r}
extract_ICC <- function(mlm.model) {
  df.icc <- data.frame(l1.var = insight::get_variance_residual(mlm.model),
                       l2.var = insight::get_variance_intercept(mlm.model)) %>%
    mutate(t.var = sum(l1.var, l2.var))
  
  df.icc %>%
    mutate(icc = l2.var / t.var)
}
...
df.icc <- df %>%
  ...
  group_by(vnames) %>%
  nest() %>%
  mutate(mlm = map(data, lme4::lmer, formula = value ~ (1|strata)),
         icc = map(mlm, extract_ICC))
