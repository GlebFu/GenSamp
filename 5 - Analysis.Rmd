...
tabs$smd.by.scale <- df.list$smd.results %>%
  ...
  mutate(plots = pmap(.l = list(data = smd.plot.data, g.title = Variables), .f = plot_smd, y.var = "abs.mSMD", l.pos = "bottom"),
...
test <- tabs$intercepts %>%
  select(-Int) %>%
  right_join(df.list$smd.results,
             by = "RR") %>%
  left_join(df.list$coefs,
            by = c("vnames", "Variables")) %>%
  mutate(y.val = SB * log_odds + Intercept)

test %>%
  filter(vnames == "Suburban") %>%
  ggplot(aes(x = y.val, y = abs.mSMD, color = as.factor(SB))) +
  geom_point() +
  geom_line() +
  facet_wrap(~Method_Label)

test %>%
  group_by(Method_Label, vnames) %>%
  arrange(mSMD) %>%
  mutate(rank = 1:n()) %>%
  filter(vnames == "Suburban") %>%
  ggplot(aes(x = rank, y = mSMD, color = RR)) +
  geom_point() +
  geom_line() +
  facet_wrap(~ Method_Label)
...
test %>%
  filter(vnames == "Suburban") %>%
  mutate(alpha.value = abs.mSMD > .2) %>%
  ggplot(aes(x = RR, y = abs.mSMD, color = Method_Label, group = interaction(Method_Label, SB))) +
  geom_line(aes(alpha = alpha.value)) +
  scale_alpha_discrete(guide = "none", range = c(.25, 1)) +
...
```{r}
tabs$smd.examples <- df.list$smd.results %>%
...
smd.examples.sep <-
  ...
  mutate(plots = map(data, plot_smd, l.pos = "none", y.var = "abs.mSMD"))

smd.examples.sep$plots[[1]]

tabs$smd.examples %>%
  ggplot(aes(x = RR, y = abs.mSMD, color = Method_Label)) +
  geom_line(size = 1) +
...
figs$relative.counts <- 
  tabs$relative.counts %>%
  filter(SB == 1) %>%
  ggplot(aes(x = RR, y = Comp.Rate, group = Sampler, color = Method_Label, linetype = Method_Label)) +
  geom_line() +
...
figs$gini <- tabs$gini.coef %>%
  ggplot(aes(x = RR, y = Gini_Coef, group = Sampler, color = Method_Label, linetype = Method_Label)) +
  geom_point() +
  geom_line() +
...
figs$gini.coef <- 
  ...
  filter(SB == 1,
         !(Rank_Factor %in% c("X9"))) %>%
  ggplot(aes(x = c.prop.pop, y = c.prop.sel, group = Sampler, color = Method_Label, linetype = Sampler)) +
  geom_line() +
