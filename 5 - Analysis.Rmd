...
smd.test <- df.list$smd.result %>%
  select(Sampler, RR, SB, vnames, Variables, log_odds, mASMD, vratio) %>%
  mutate(Neutrality  = mASMD <= .25,
...
smd.test %>%
  ...
  mutate(alpha = Neutrality,
         shape = Neutrality == 1) %>%
  ggplot(aes(x = vratio, y = abs(log_odds), color = value, alpha = alpha, shape = shape)) +
  geom_point(size = 4 ) +
