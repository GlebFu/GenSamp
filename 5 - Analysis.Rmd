...
df.list$PS %>%
  mutate(RR = parse_number(RR) %>% as_factor()) %>%
...
Set_Compare <- function(data, variable = "Sampler", value = "S_BS_SR_X1", observations = c("M", "q_25", "q_50", "q_75")){
  data %>%
...
tabs$B <- 
  ...
  unnest(Quantile) %>%
  filter(Cluster == str_replace(Rank_Type, " Ranks", ""))
...
tabs$B %>%
  filter(SB == 1,
         Cluster == str_replace(Rank_Type, " Ranks", ""),
         Rank_Factor == "X1") %>%
  ggplot(aes(x = RR, y = M, color = Sample, linetype = Cluster)) +
  geom_line() +
...
```{r}
tabs$B %>%
  left_join(Set_Compare(.)) %>%
  filter(SB == 1,
         Sampler != Sampler_C) %>%
  ggplot(aes(x = RR, y = q_50, group = Sampler, color = Sampler)) +
  geom_line() +
  geom_line(aes(y = q_25), linetype = "dotted") +
  geom_line(aes(y = q_75), linetype = "dotted") +
  geom_line(aes(y = q_50_C), color = "black", alpha = .2) +
  # geom_errorbar(aes(ymin = q_25, ymax = q_75), width = 3) +
  # geom_errorbar(aes(ymin = q_25_C, ymax = q_75_C), color = "black", width = .2, alpha = .2) +
  geom_hline(yintercept = c(.8, .95), linetype = "dotted", alpha = .5) +
  facet_wrap(Cluster ~ Sampler, nrow = 2) +
  theme_minimal() +
  theme(legend.position = "none")
...
tabs$B %>%
  filter(SB == 1,
         Sample == "Convenience") %>%
  ggplot(aes(x = RR, y = M, group = Sampler, color = Rank_Factor)) +
  geom_line() +
  geom_line(aes(y = q_25), linetype = "dotted") +
  geom_line(aes(y = q_75), linetype = "dotted") +
  # geom_errorbar(aes(ymin = q_25, ymax = q_75), width = 3) +
  geom_hline(yintercept = c(.8, .95), linetype = "dotted", alpha = .5) +
  facet_grid(~ Rank_Type + Cluster) +
  theme_minimal() +
  theme(legend.position = "none")
...
df.list$smd.results <- results$smd.stats %>%
  select(SB:vnames, Variables, log_odds, Sample_Catagories, mSMD) %>%
  unpack(Sample_Catagories)
...
df.list$smd.results %>%
  filter(SB == 1,
         ...
         Rank_Factor == "X1",
         strata == 0,
         !(vnames %in% c("pFem", "ST.ratio"))) %>%
  ggplot(aes(x = RR, y = mSMD, color = Sample, linetype = Cluster)) +
  geom_line(size = 1) +
  facet_wrap(~Variables, scales = "free") +
  geom_hline(yintercept = 0) +
  geom_hline(aes(yintercept = log_odds), linetype = "dashed") +
...
df.list$smd.results %>%
  filter(Cluster == str_replace(Rank_Type, " Ranks", ""),
         ...
         Rank_Factor == "X1",
         # strata != 0,
         vnames %in% c("ethBlack")) %>%
  mutate(strata = paste("Strata", strata)) %>%
  ggplot(aes(x = RR, y = mSMD, color = strata)) +
  geom_line() +
  facet_grid(SB ~ Sample + Cluster, scales = "free_y") +
  geom_hline(yintercept = 0) +
  geom_hline(yintercept = c(.25, -.25), linetype = "dotted") +
  theme_minimal() +
  theme(legend.position = "bottom",
...
df.list$smd.results %>%
  filter(Sampler == "S_CS_SR_X1",
         strata != 0) %>%
  mutate(strata = paste("Strata", strata)) %>%
  ggplot(aes(x = RR, y = abs(mSMD), color = strata)) +
  geom_line() +
  facet_grid(SB ~ Variables, scales = "free_y") +
  geom_hline(yintercept = 0) +
...
df.list$smd.results %>%
  ...
  ggplot(aes(x = RR, y = mSMD, color = strata)) +
  geom_line() +
  facet_grid(Variables ~ Sample + Cluster, scales = "free_y") +
  geom_hline(yintercept = 0) +
  geom_hline(yintercept = c(.25, -.25), linetype = "dotted") +
  theme_minimal() +
  theme(legend.position = "bottom",
...
df.list$smd.results %>%
  filter(SB == 1,
         ...
         Rank_Factor == "X1",
         strata != 0,
         vnames %in% c("Suburban", "Urban", "ToRu")) %>%
  mutate(strata = paste("Strata", strata)) %>%
  ggplot(aes(x = RR, y = mSMD, color = strata)) +
  geom_line() +
  facet_grid(Variables ~ Sample + Cluster, scales = "free_y") +
...
smd.test <- df.list$smd.result %>%
  select(Sampler, RR, SB, vnames, Variables, log_odds, mSMD, vratio) %>%
  mutate(Neutrality  = (-.25 <= mSMD) & (mSMD <= .25),
         log_odds = SB * log_odds) %>%
  group_by(SB, vnames, Variables, log_odds, vratio) %>%
  mutate(Neutrality  = mean(Neutrality )) %>%
  spread(Sampler, mSMD) %>%
  mutate(SBS_vs_Strata = SBS <= min(c(SCS, SRS, SC2)),
         SBS_vs_All = SBS <= min(c(SCS, SRS, SC2, URS, UCS)),
         SCS_vs_All = SCS <= min(c(SBS, SRS, SC2, URS, UCS)),
         Strata_in_CS = SCS <= UCS,
         Strata_in_CS2 = SC2 <= UCS,
         Strata_in_RS = SRS <= UCS)
smd.test %>%

  group_by(Variables) %>%
  mutate(Mean_Neutrality = mean(Neutrality )) %>%
  filter(Mean_Neutrality == 1)

smd.test %>%
  select(RR, SB, vnames, Variables, log_odds, vratio, Neutrality, SBS_vs_Strata:Strata_in_RS) %>%
  gather(comparison, value, SBS_vs_Strata:Strata_in_RS) %>%
  mutate(alpha = Neutrality,
         shape = Neutrality == 1) %>%
  ggplot(aes(x = vratio, y = log_odds, color = value, alpha = alpha, shape = shape)) +
  geom_point(size = 4 ) +
...
smd.test %>%
  ...
  mutate(alpha = Neutrality,
         shape = Neutrality == 1) %>%
  ggplot(aes(x = RR, y = log_odds, color = value, alpha = alpha, shape = shape)) +
  geom_point(size = 4 ) +
  facet_wrap(~comparison) +
  theme_bw() +
  scale_shape_manual(values=c(20, 1))
...
# smd.test <- df.list$smd.result %>%
#   select(RR, SB, Variables, vnames, Sampler, mASMD) %>%
#   spread(Sampler, mASMD) %>%
#   mutate(CS_Dif_1 = UCS - SCS,
#          CS_Dif_2 = UCS - SC2,
#          RS_Dif = URS - SRS,
#          BS_Dif_R = SRS - SBS,
#          BS_Dif_C1 = SCS - SBS,
...
# 
# smd.test %>%
#   ggplot(aes(x = RR, y = CS_Dif_1, color = SB)) +
#   geom_line() +
#   geom_hline(yintercept = 0) +
...
# 
# smd.test %>%
#   ggplot(aes(x = RR, y = CS_Dif_2, color = SB)) +
#   geom_line() +
#   geom_hline(yintercept = 0) +
#   facet_wrap(~Variables, scale = "free") +
#   theme_minimal()
...
df.list$smd.results %>%
  filter(SB == 1,
         vnames == "n") %>%
  ggplot(aes(x = RR, y = mASMD, color = Sampler)) +
  geom_line() +
  theme_minimal()
...
#   select(Sampler, RR, SB, vnames, mASMD, Variables) %>%
#   nest(data = -c(vnames, Variables)) %>%
#   mutate(resid = map(data, function(x) lm(mASMD ~ RR + I(RR^2), data = x) %>% residuals())) %>%
#   unnest(cols = c(data, resid))
# 
# test %>%
#   ggplot(aes(x = RR, y = resid, color = Sampler, group = interaction(Variables, Sampler))) +
#   geom_line() +
#   facet_grid(SB ~ Variables)
...
tabs$smd.label <- tibble(Sampler = c("SBS", "SRS", "SCS", "UCS", "URS"),
                       mASMD = (1:5) * .18, 
                       Variables = "Sampler",
                       xmin = 10, 
                       xmax = 90) %>%
  gather(key = lim, value = RR, xmin, xmax) %>%
  arrange(desc(Sampler))

figs$smd.label <- tabs$smd.label %>%
  ggplot(aes(x = RR, y = mASMD, color = Sampler, linetype = Sampler, group = Sampler, label = Sampler)) +
  geom_line(size = 1) +
...
tabs$relative.counts <- tabs$samples  %>%
  ...
  select(-Sampler) %>%
  spread(key = Sampler, value = value) %>%
...
figs$relative.counts <- tabs$relative.counts %>%
  ggplot(aes(x = RR, y = Comp.Times, group = Sampler, color = Sampler, linetype = Sampler)) +
  geom_line() +
...
tabs$gini.coef.labels <-  results$gini.coef %>%
  mutate(RR = parse_number(RR)) %>%
  filter(RR %in% c(10)) %>%
  mutate(RR = 5) %>%
  arrange(gini) %>%
...
figs$gini <- tabs$gini.coef %>%
  mutate(RR = parse_number(RR)) %>%
  ggplot(aes(x = RR, y = gini, group = Sampler, color = Sampler, linetype = Sampler)) +
  geom_point() +
  geom_line() +
...
figs$gini.coef <- tabs$gini.count %>%
  # filter(RR == "RR_90") %>%
  # filter(Sampler == "Unstratified_RS") %>%
  ggplot(aes(x = c.prop.pop, y = c.prop.sel, group = Sampler, color = Sampler, linetype = Sampler)) +
  geom_line() +
