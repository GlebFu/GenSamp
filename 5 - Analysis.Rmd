...
```{r}
tabs$B %>%
  mutate(Rank_Type = str_replace(Rank_Type, " Ranks", "")) %>%
  filter(SB == 1,
         C)
...
tabs$B %>%
  filter(SB == 1) %>%
  ggplot(aes(x = RR, y = M, color = Rank_Factor, linetype = Cluster)) +
  geom_line() +
  facet_wrap(~ SB) +
  geom_hline(yintercept = c(.8, .95), linetype = "dotted", alpha = .5) +
  facet_wrap(~ Sample + Rank_Type) +
  theme_minimal()
```
...
tabs$B %>%
  filter(SB == 1) %>%
  unnest(Quantile) %>%
  ggplot(aes(x = RR, y = q_50, color = interaction(Sample, Rank_Type))) +
  geom_line() +
...
#   geom_hline(yintercept = .8, linetype = "dashed") +
#   scale_x_continuous(breaks = seq(0, 100, 10)) + 
#   scale_y_continuous(limits = c(0.65, 1.0), expand = c(0,0)) +
#   facet_grid(rows = vars(SB), scales = "free_y")
# 
# 
# figs$B 
# 
...
df.list$smd.results %>%
  ...
  # mutate(mSMD = abs(mSMD),
  #        log_odds = abs(log_odds)) %>%
  ggplot(aes(x = RR, y = mSMD, color = Sampler)) +
  geom_line(size = 1) +
