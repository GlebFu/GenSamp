...
```{r}
vars$smd.groups <- 
  tibble(`Group 1` = list(c("Suburban", "T1", "Urban")),
         `Group 2` = list(c("pTotfrl", "ethHisp", "ethWhite", "pELL", "dSCH", "ToRu")),
         `Group 3` = list(c("n", "ethBlack")),
         `Group 4` = list(c("pFem", "ST.ratio"))) %>%
...
test.g1 <-
  ...
  summarize(g1_a = prod(below_25),
            g1_b = prod(Cluster == "Stratified"),
            g1_c = 1 - g1_b,
            .groups = "drop_last") %>%
  summarise(g1_ab = sum(g1_a * g1_b),
            g1_ac = 1 - sum(g1_a * g1_c),
            .groups = "drop") %>%
  filter(g1_ab * g1_ac == 1) %>%
...
# Not in Groups 1 or 4

test.g2 <-
  test.group %>%
  group_by(SB, vnames, Method) %>%
  mutate(g2_a = (Cluster != comp_Cluster) & 
           (Sample == comp_Sample) & 
           (Cluster == "Stratified"),
         g2_b = (Method == "SBS") &
           (comp_Cluster == "Unstratified")) %>%
  filter(g2_a | g2_b) %>%
  summarise(g2_c = prod(less),
            .groups = "drop_last") %>%
  summarise(g2_abc = prod(g2_c),
            .groups = "drop") %>% 
  filter(g2_abc == 1) %>%
  anti_join(test.g1, 
            by = c("SB", "vnames")) %>%
...
```{r}
figs$smd.by.scale[unique(test.g3$vnames)]
