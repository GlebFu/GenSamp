...
df.list$smd.results <- results$smd.stats %>%
  ...
  filter(!(Rank_Factor %in% c("X9")),
         strata == 0) %>%
  select(-K, -strata, -log_odds)

tabs$smd.by.scale <- df.list$smd.results %>%
  nest(smd.plot.data = c(SB, RR, Sampler, Method_Label:abs.mSMD)) %>%
  mutate(plots = pmap(.l = list(data = smd.plot.data, g.title = Variables), .f = plot_smd2, yvar = "abs.mSMD", l.pos = "bottom"),
...
df.list$smd.by.strata.results %>%
  ...
  geom_line() +
  facet_grid(Variables ~ Cluster + Sample , scales = "free_y") +
...
#   select(Sampler, RR, SB, vnames, Variables, log_odds, mSMD) %>%
#   mutate(Neutrality  = (-.25 <= mSMD) & (mSMD <= .25),
#          log_odds = SB * log_odds) %>%
#   group_by(SB, vnames, Variables, log_odds) %>%
...
figs$samples.contacts <- 
  tabs$samples %>%
  filter(SB == 1) %>%
  ggplot(aes(x = RR, y = contacted, group = Sampler, color = Sampler, linetype = Sampler)) +
  geom_point() +
  geom_line() +
  theme(legend.position = "none") +
  scale_x_continuous(breaks = seq(0, 100, 10)) +
...
tabs$samples.response.rates.labels <-  
  ...
  group_by(Sampler) %>%
  summarise(response_rate = (min(response_rate) + (max(response_rate) - min(response_rate)) / 2) * 100) %>%
  mutate(RR = 45) %>%
...
tabs$relative.counts <- 
  tabs$samples  %>%
  ungroup() %>%
  select(SB:strata, Sampler, response_rate) %>%
  spread(key = Sampler, value = response_rate) %>%
  right_join(tabs$samples) %>%
  gather(key = Comp.Method, value = Comp.Value,
         contains("_RS"), contains("_BS"), contains("_CS")) %>%
  mutate(Comp.Rate = response_rate / Comp.Value) 

figs$relative.counts <- 
  tabs$relative.counts %>%
  filter(SB == 1) %>%
  ggplot(aes(x = RR, y = Comp.Rate, group = Sampler, color = Sampler, linetype = Sampler)) +
  geom_line() +
...
tabs$gini.coef.labels <-
  ...
  mutate(RR = 5) %>%
  arrange(Gini_Coef) %>%
  mutate(Gini_Coef = c(.5, .65, .75, .95, .85))
```
...
figs$gini <- tabs$gini.coef %>%
  ggplot(aes(x = RR, y = Gini_Coef, group = Sampler, color = Sampler, linetype = Sampler)) +
  geom_point() +
  geom_line() +
...
save(df.list,
     ...
     tabs,
     vars,
     file = "Data/Paper/Paper Data 01-14-2021.rdata")
```
