...
df.list$smd.results  %>%
  ggplot(aes(x = RR, y = mASMD, group = interaction(Variables, SB, Sampler), color = Sampler)) +
  geom_line(alpha = .5) +
  theme(legend.position = "none")
...
smd.test <- df.list$smd.result %>%
  mutate(strat = str_sub(Sampler, end = 1),
         type = str_sub(Sampler, start = -2)) %>%
  group_by(type, Variables, RR, SB) %>%
  arrange(strat) %>%
  summarise(smd_dif = diff(mASMD))

smd.test %>%
  filter(type == "CS") %>%
  ggplot(aes(x = RR, y = smd_dif, color = as.factor(SB))) +
  geom_line() +
...
#   mutate(resid = map(data, function(x) lm(mASMD ~ 0 + RR + I(RR^2), data = x) %>% residuals())) %>%
#   unnest(cols = c(data, resid))
# 
# smd.test %>%
#   ggplot(aes(x = SB, y = resid, group = interaction(RR, Sampler), color = Sampler)) +
#   geom_line(alpha = .5) +
#   theme(legend.position = "none") +
#   facet_wrap(~ Variables)
...
test <- df.list$smd.results %>%
  nest(cols = -Variables) %>%
  mutate(plots = map(cols, plot_smd),
         plots = set_names(plots, paste("smd.", Variables, sep = "")),
         data = set_names(cols, paste("smd.", Variables, sep = "")))
test$plots
...
```{r}
df.test %>%
  ggplot(aes(x = abs_log_odds, y = mASMD, color = Sampler)) +
  geom_point() +
  geom_hline(yintercept = .25, linetype = "dotted") +
...
df.test %>%
  ggplot(aes(x = log_odds, y = mSMD, color = Sampler)) +
  geom_point() +
  geom_hline(yintercept = .25, linetype = "dotted") +
  geom_hline(yintercept = 0) +
  geom_smooth(method = "lm", se = F) +
...
```{r}
df.test %>%
  ggplot(aes(x = Sampler, y = mASMD, color = Sampler)) +
...
df.plot %>%
  ...
  unnest(plot.data) %>%
  ggplot(aes(x = RR, y = res, color = Sampler, linetype = Sampler)) +
  # geom_point(alpha = .5) +
  geom_smooth() +
  facet_wrap(~model)
...
df.plot %>%
  select(model, plot.data) %>%
  unnest(plot.data) %>%
  ggplot(aes(x = vratio, y = res, color = Sampler)) +
  geom_point() +
  geom_hline(yintercept = 0) +
  facet_grid(model ~ RR)
...
df.test.2.sum %>%
  ggplot(aes(x = mSMD, y = mSMD, color = Sampler)) +
  geom_point() +
...
df.test.2 %>%
  filter(var == "ethWhite") %>%
  ggplot(aes(x = RR, y = a_smd, color = Sampler)) +
  geom_smooth(method = "lm", formula = y ~ poly(x, 3))

df.test.2 %>%
  filter(var == "ethWhite") %>%
  ggplot(aes(x = RR, y = smd, color = Sampler)) +
  geom_smooth(method = "lm", formula = y ~ poly(x, 3))
```
