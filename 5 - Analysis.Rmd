...
df.test <- bind_rows(tabs$smd.bad, tabs$smd.good1, tabs$smd.good2, tabs$smd.neutral) %>%
  select(sample_method:mSMD) %>%
  left_join(tabs$v.coef %>% select(vnames, vratio, log_odds)) %>%
  mutate(rat1 = vratio/abs(log_odds) %>% log,
         rat2 = abs(log_odds)/vratio)

df.test %>%
  ggplot(aes(x = RR, y = mSMD, color = Clustering, group = sample_method)) +
  geom_line() +
  facet_wrap(~ rat1) +
...
df.test %>%
  ggplot(aes(x = vratio, y = mSMD, alpha = log_odds, color = Clustering)) +
  geom_point() +
  facet_grid( ~ RR) +
  geom_hline(yintercept = .25, linetype = "dotted") +
  scale_y_log10()
