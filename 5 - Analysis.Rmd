...
tabs$pop.desc <- 
  ...
  mutate(M = round(pop.mean, 2),
         ...
         SF050 = (log_odds * .5) %>% round(3),
         SF025 = (log_odds * .25) %>% round(3),
         SF001 = (log_odds * .001) %>% round(3))
...
test <- 
  ...
  ungroup() %>%
  unique() %>%
  mutate(abs_log_odds = abs(Coeficient)) %>%
...
test.relative.values <- 
  ...
  ungroup() %>%
  mutate(rel_vrat = (vratio - mean(vratio)) / sd(vratio),
