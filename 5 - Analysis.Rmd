...
get_quant <- function(x) tibble(Q = quantile(x), Percent = paste("q_", parse_number(names(Q)), sep = "")) %>% spread(Percent, Q)
...
tabs$B %>%
  filter(SB == 1) %>%
  unnest(Quantile) %>%
  ggplot(aes(x = RR, y = q_50, color = Sampler)) +
  geom_line() +
